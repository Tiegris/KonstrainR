...
class SslServiceOpenSslWrapperImpl : SslService {
    private val pwd = File("/kscore/ssl")
    ...
    init {
        ...
        if (!File(pwd, "rootCA.crt").exists())
            shell {
                openssl("req -x509 -new -nodes -key rootCA.key -sha256 -days 365 -out rootCA.crt -subj \"/C=HU/O=me.btieger/CN=konstrainer-core\"")
            }
    ...
    override fun deriveCert(agentServiceName: String): SecretBundle {
        ...
        shell {
            ...
            openssl("req -new -sha256 -key $keyName -subj \"/C=HU/O=me.btieger/CN=$agentServiceName\" -out $csrName")
            openssl("x509 -req -days 365 -in $csrName -out $certName -CAkey rootCA.key -CA rootCA.crt")
        }
    ...
    private fun ShellScript.openssl(args: String) = command("openssl", listOf(args))
